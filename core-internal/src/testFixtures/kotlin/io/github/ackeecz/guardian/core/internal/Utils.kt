...
public val String.utf8ByteSize: Int
    get() = toByteArray(Charsets.UTF_8).size

/**
 * Returns [ByteBuffer.remaining] number of bytes in this [ByteBuffer] as [ByteArray]
 */
public fun ByteBuffer.remainingToByteArray(): ByteArray {
    return ByteArray(remaining()).also { get(it) }
}

public infix fun ByteString.shouldHaveBitSize(bitSize: Int) {
    toByteArray()
        .size
        .let { it * 8 }
        .shouldBe(bitSize)
...
public fun Context.getKeysetGetCallCount(prefsName: String, keysetAlias: String): Int {
